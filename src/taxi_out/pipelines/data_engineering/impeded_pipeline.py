...
# each pipeline is :    imp_pip_core_nodes+ imp_extra_nodes+imp_{}_extra_nodes,
# I could merge pip_core and extra ??
# node(
#     func=set_index,
...
def create_pipeline(**kwargs):
    ...
    imp_full_extra_nodes = Pipeline(
        [
            ...
            node(
                ...
                inputs=["data_amafiltered_imp_full",
                        "params:NIQR_FULL_TAXI",
                ],
                outputs="data_fullfiltered_imp_full",
            ),            
            node(
                func=apply_filter_req_niqr_dep_ramp_taxi_times,
                inputs=[
                    "data_fullfiltered_imp_full",
                    "params:NIQR_RAMP_TAXI",
                ],
                outputs="data_engred_imp_full",
            ),
            node(
                func=train_test_group_logging,
                inputs="data_engred_imp_full",
                outputs=None,
            ),
        ]
    )
    ...
    imp_ramp_extra_nodes = Pipeline(
        [
            node(
                ...
                outputs="data_engred_imp_ramp",
            ),
            node(
                func=train_test_group_logging,
                inputs="data_engred_imp_ramp",
                outputs=None,
            ),
        ]
    )
    ...
    return {
        ...
        imp_pip_core_nodes+
        imp_extra_nodes+
        imp_full_extra_nodes,
        'imp_ramp_pip':
        imp_pip_core_nodes+
        imp_extra_nodes+
        imp_ramp_extra_nodes,
        'imp_ama_pip':
        imp_pip_core_nodes+
        imp_extra_nodes+
        imp_ama_extra_nodes,
